...
// 0.6.0.0 : Intégration MP Storage
...
var LocalMPStorage = {
    ...
    hasFlagMulti: function(postId){
        ...
        console.warn('hasFlagMulti ?', postId, LocalMPStorage.flags.list.filter(function(item){ return postId === item.post; }).length > 0);
    ...
    getFlagMulti: function(postId){
        ...
        console.warn('getFlagMulti ?', postId, LocalMPStorage.flags.list.filter(function(item){ return postId === item.post; })[0]);
...
LocalMPStorage.initMultiMPStorage().then(function(){
    ...
    /*if (HFR.getValue ("hfr-multimp-initialized") != "oui")
        mpStorage.initStorage (res => {
            HFR.setValue ("hfr-multimp-mp-" + HFR.getCookie ("md_user"), {
                mp_rep_id : mpStorage.mpRepId,
                mp_id : mpStorage.mpId
            });
            HFR.setValue ("hfr-multimp-initialized", "oui");
            LocalMPStorage.getData(function(flags){
                HFR.setValue("hfr-multimp-flags", flags);
            });
        });
    else {
        var obj = HFR.getValue ("hfr-multimp-mp-" + HFR.getCookie ("md_user"));
        mpStorage.mpId = obj.mp_id;
        mpStorage.mpRepId = obj.mp_rep_id;
        mpStorage.username = HFR.getCookie ("md_user");
        LocalMPStorage.getData(function(flags){
            HFR.setValue("hfr-multimp-flags", flags);
        });
    }*/
    ...
    if (document.location.href.indexOf("https://forum.hardware.fr/forum1.php") === 0) {
        console.warn('On est dans les MP');
