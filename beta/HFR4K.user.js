...
// 2020.01.07.0 : Ezzzi Draps; le click droit ouvre dans un nouvel onglet.
// 2020.01.05.1 : Fix issue #17 pour compatibilité [HFR] New Page Number.
...
var HFRGMUtils = {
    ...
    isNewVersion: function(){
        ...
        // Ugly but finally does the job...
        var result = false;
        var done = false;
        var localArr = version.split('.');
        LocalMPStorage.datas.hfr4k.lastVersion.split('.').forEach(function(n, i){
            if(!done){
                if (parseInt(n) < parseInt(localArr[i])){
                    result = true;
                    done = true;
                } else if (parseInt(n) > parseInt(localArr[i])){
                    done = true;
                }
            }
    ...
    isAnotherOlderVersion: function(){
        // **********
        // HFR4K_GM
        // Check if the current version of the script older than the newest used (the user use HFR4K on multiple computers, but this one is not up to date)
        // Return boolean
        // **********
    ...
    getCleanPseudo: function(pseudo){
        // **********
        // HFR4K_GM
        // Clean username by removing special char
        // **********
    ...
    getSimplePseudo: function(pseudo){
        // **********
        // HFR4K_GM
        // Clean username to simplify comparisons (remove special char, spaces and do a lowercase)
        // **********
    ...
    addToBlacklist: function(pseudo){
        // **********
        // HFR4K_GM
        ...
        LocalMPStorage.datas.blacklist.list.push(entry);
        LocalMPStorage.datas.blacklist.list.sort();

        // Update MPStorage
        LocalMPStorage.updateMPStorage(LocalMPStorage.domains.blacklist);

    },
    removeFromBlacklist: function(pseudo){
        // **********
        // HFR4K_GM
        ...
        // Find pseudo
        let i = LocalMPStorage.datas.blacklist.list.findIndex(function(d){ return HFRGMUtils.getSimplePseudo(pseudo) === HFRGMUtils.getSimplePseudo(d.username); });

        if(i >= 0) {
            // Remove the entry from the list
            LocalMPStorage.datas.blacklist.list.splice(i, 1);
    ...
    getUnreadMPs: function(){
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                mpStorage.makeWebRequest(url, 'get', void 0, function (resp) {
...
var HFR4K = {
    ...
    fullRender: function(){
        ...
        modal.id = 'hfr4kModal';
        modal.classList.add('hfr4kModal');
        let modCont = document.createElement('div');
        modCont.classList.add('hfr4kModalContent');
        modal.appendChild(modCont);
        document.querySelector('div.container').parentNode.appendChild(modal);
        ...
        if(HFRGMUtils.isNewVersion()){
            // This version is newer than the last used
            if(mpHead != null) {
        ...
        }else if (HFRGMUtils.isAnotherOlderVersion()){
            // This version is older than the last used (probably on another computer)
            if(mpHead != null) {
                // Create New version message
                let divMessageVersion = document.createElement('div');
                divMessageVersion.classList.add('left', 'hfr4kUpdateBox');
                divMessageVersion.onclick = function(){
                    // Send him to the changelog
                    GM.openInTab('https://wiripse.github.io/HFRGMTools/#mise-%C3%A0-jour');
                };
                let imgUpgradeVersion = document.createElement('img');
                imgUpgradeVersion.setAttribute('src', HFRGMUtils.icons.update);
                imgUpgradeVersion.style.verticalAlign = 'bottom';
                divMessageVersion.appendChild(imgUpgradeVersion);
                divMessageVersion.appendChild(document.createTextNode('\u00a0'));
                divMessageVersion.appendChild(document.createTextNode('HFR4K n\'est pas à jour !'));
                mpHead.appendChild(divMessageVersion);
            }
        }
    ...
    createTableSettings: function(classes, elementAfter){
        ...
        // *** Liste Noire *** //
        // *************************************************************************************** //
        taBody.appendChild(HFR4K.getCatSetting('Liste Noire (La Mer)'));

        // Setting Liste Noire Activation
        taBody.appendChild(HFR4K.getCheckboxSetting('Activer ?', LocalMPStorage.datas.blacklist.active, function(cb){
            LocalMPStorage.datas.blacklist.active = !LocalMPStorage.datas.blacklist.active;
            cb.checked = LocalMPStorage.datas.blacklist.active;
            LocalMPStorage.updateMPStorage(LocalMPStorage.domains.blacklist);
        }));
    ...
    renderModal: function(content, onConfirm){
        // **********
        // HFR4K_GM
        // Display modal with given content
        // onConfirm : Method called when user valid actio
        // **********

        // Get the modal in the page
        let modal = document.getElementById('hfr4kModal');
        let modCont = modal.querySelector('div');
        // Reset content
        modCont.innerHTML = '';
        // Append content
        modCont.appendChild(content);

        // Add buttons if necessary
        if(onConfirm){
            let pIcons = document.createElement('p');
            let iconConfirm = document.createElement('img');
            iconConfirm.setAttribute('src', HFRGMUtils.icons.check);
            iconConfirm.style.cursor = 'pointer';
            iconConfirm.style.padding = '3px';
            iconConfirm.onclick = function(){
                // Call the method
                onConfirm();
                // Close modal
                modal.style.display = 'none';
            }
            pIcons.appendChild(iconConfirm);
            let iconCancel = document.createElement('img');
            iconCancel.setAttribute('src', HFRGMUtils.icons.uncheck);
            iconCancel.style.cursor = 'pointer';
            iconCancel.style.padding = '3px';
            iconCancel.onclick = function(){
                // Close modal
                modal.style.display = 'none';
            }
            pIcons.appendChild(iconCancel);
            modCont.appendChild(pIcons);
        }

        // Content initiated, we display the modal
            ...
            if(LocalMPStorage.datas.superFavs.settings.active){
                ...
                topicRows.forEach(function(topicRow){
                    ...
                    if(!topicRow.classList.contains('privateMessage') && !topicRow.classList.contains('hfr4kSettingRow') && topic){
                        ...
                        var topicUrl = new URLSearchParams(topic.querySelector('a').hasAttribute('href') ? topic.querySelector('a').href : topic.querySelector('a').dataset.href);
            ...
            if(document.querySelector('table.hfr4kMainTab')){
                document.querySelector('table.hfr4kMainTab').querySelectorAll('tr.cBackHeader.fondForum1fCat:not(.hfr4kMPCat)').forEach(function(cat){
                    ...
                    while(!nextCat && curSibling.nextSibling){
                        ...
                        hasTopics = curSibling.classList.contains('sujet') && curSibling.classList.length > 1;;
    ...
    renderEasyFlags: function(){
        // **********
        // HFR4K_GM
        // Bigger flag cell, all clickable (RightClick or CTRL+Click or mollette click to open in a new tab)
            ...
            document.querySelectorAll('tr.sujet.ligne_booleen').forEach(function(topic){
                ...
                if(!topic.classList.contains('hiddenFav')){
                    ...
                    if(caseDrapal && caseDrapal.querySelector('a')){
                        // Get drapal's URL
                        let drapURL = caseDrapal.querySelector('a').hasAttribute('href') ? caseDrapal.querySelector('a').href : caseDrapal.querySelector('a').dataset.href;
                        // Add special style
                ...
                Array.from(document.querySelectorAll('.toolbar')).filter(tb => {
                    ...
                    imgDel.onclick = function(e){
                        ...
                        if (confirm('Supprimer ce post ?')){
                            ...
                            // Make the request
                            mpStorage.makeWebRequest('https://forum.hardware.fr/bdd.php?config=hfr.inc', 'post', argz, function (resp) {
    ...
    renderListeNoire: function(){
        // **********
        // HFR4K_GM
        // Render ListeNoire at first load
        // **********
    ...
    simpleRenderListeNoire: function(){
        // **********
        // HFR4K_GM
        // Render ListeNoire on the topic
        // **********
        ...
        document.querySelectorAll('.hfr4kEmptyBL').forEach(function(emptyItem){
        ...
        if(LocalMPStorage.datas.blacklist.active && (HFRGMUtils.pageCheck.isOnMP() || HFRGMUtils.pageCheck.isOnTopic() || HFRGMUtils.pageCheck.isOnTopicTxt() ||
            ...
            document.querySelectorAll('tr.message').forEach(function(post){
                ...
                if(HFRGMUtils.isPseudoBlacklisted(pseudo)){
                    ...
                    emptyRow.classList.add('hfr4kEmptyBL');
            ...
            document.querySelectorAll('a.Topic').forEach(function(quoteLink){
                ...
                if(quoteLink.textContent.split(' a écrit').length === 2 && HFRGMUtils.isPseudoBlacklisted(pseudo) &&
                   (quoteLink.closest('table.citation') || quoteLink.closest('table.oldcitation'))){
                    ...
                    emptyQuote.classList.add('hfr4kEmptyBL');
        ...
        if (LocalMPStorage.datas.mpFlags.active){
            if (HFRGMUtils.pageCheck.isOnMP()){
                ...
                if(HFRGMUtils.hasFlagDT(mpPostID)){
                    ...
                    // Post request
                    mpStorage.makeWebRequest(url, 'post', void 0, function (resp) {
